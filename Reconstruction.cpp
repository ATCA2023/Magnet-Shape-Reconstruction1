...
double computePointField(const SurfacePoint& dipolePoint, const Eigen::Vector3d& position)
{
    ...
    double dipoleSeparation = 0.0001;  // Separation distance between positive and negative poles of the dipole
                                    ///  dipoleSeparation and smallestradius must me modified for all shapes to make sure that
                                    ///  numpoints/2* precalculted[] is (observably) smaller than the force of one dipole at the closest measurement
...
int main()
{
    ...
    const int SurfaceSize=surfacePoints.size();
    ...
    for(double scale=maxscale ; scale>=minscale; scale*=speed)
    {
                ...
                if (std::isfinite(surfacePoint.x) && std::isfinite(surfacePoint.y) && std::isfinite(surfacePoint.z))
                {
                    ...
                    for (size_t j = 0; j < recordedPoints2.size(); ++j)
                    {
                        ...
                        if (Signchange)
                        {
                            ...
                            else changeability[first[j]][j]=1, changeability[i][j]=1;
                        }
                    }
        ...
        vector <int> specialcases;

        for(size_t i=0; i<recordsize; i++) if(!changeabilityver[i]) specialcases.push_back(i); // Never modified
        ...
        for(size_t i=0; i < specialcases.size(); i++) changeabilityver[specialcases[i]]=1;

        fixedver.reset();

        for(size_t i=0; i<surfacePoints2.size(); i++)      //Save fixedver
        {
            bool cnt=false;
            ...
            for (size_t i = 0; i < surfacePoints.size(); ++i)
            {
                ...
                if (std::isfinite(surfacePoint.x) && std::isfinite(surfacePoint.y) && std::isfinite(surfacePoint.z))
                {
                    ...
                    if(isinsiderecorded)
                    {
                        ...
                        continue;                       // Should be break but we don't have data from all around the core
                    }
                    ...
                    for (size_t j = 0; j < recordedPoints.size(); ++j)
                    {
                        ...
                        int a = 0;
                        for (int i = SurfaceSize - 1; i >= 0; --i) if (state[i] != fixedver[i]) a=state[i] ? 1 : -1;

                        if (a == 1)        /// If there still are fixed points uncalculated yet
                        {
                            if(changeabilityver[j]) continue;

                            if(!fixedver[i] && recordedPoint.intensity * modifiedIntensities[j] > 0)   /// If we have the same sign and a fixed point
                            {
        ...
        for (size_t i = 0; i < recordsize; ++i)
        {
            ...
            {
                if(state[j]==1) if (changeability[j][i]) recordedPointOrder[k]++, sortedrecordedPointIndices[k] = i;
            }
        ...
        {
            for(size_t j=0; j<surfacePoints.size(); j++) if(changeability[j][sortedrecordedPointIndices[i]] && state[j]==1) surfaceOrder[j]=1;
            cout<<i;
        ...
        while (isModified)
        {
            ...
            {
                if(!changeabilityver[recordedIndex]) continue;
                ...
                for (auto& surfaceIndex : surfacePointfinalOrder)
                {
                    ...
                    if (std::isfinite(surfacePoint.x) && std::isfinite(surfacePoint.y) && std::isfinite(surfacePoint.z))
                    {
                        ...
                        {
                            changeabilityver[recordedIndex]=0;
                            surfaceOrder.reset();
